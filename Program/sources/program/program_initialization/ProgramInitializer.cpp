		...
		argv_(argv)
{
	...
	runOptions_.add_options()
            ...
            ( command( RESULT, "r" ).c_str(), value< std::string >( &resultPath_ )->default_value( "./" ), "Specifies result path" )
            ( command( LOGGER, "l" ).c_str(), value< std::string >( &loggerFile_ )->default_value( "./logger.csv" ), "Specifies logger file" )
